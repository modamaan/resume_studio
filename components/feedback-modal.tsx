...
export function FeedbackModal({ open, onOpenChange }: FeedbackModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const moodText = mood === 'happy' ? 'Loved it 😊' : mood === 'neutral' ? 'It was okay 😐' : mood === 'sad' ? 'Needs work ☹️' : '';
    const ratingText = rating ? `${rating}/5 stars` : 'No rating';
    const userEmail = user?.primaryEmailAddress?.emailAddress || 'Anonymous';

    const subject = `Feedback: ${moodText}`;
    const body = `
...
Submitted at: ${new Date().toLocaleString()}
    ...
    // Create mailto link
    const mailtoLink = `mailto:amaanprogramming@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-lg rounded-2xl bg-white shadow-xl border border-gray-100">
        ...
        <div className="px-5 py-4 sm:py-5">
          {status && (
            <div
              className={`mb-4 flex items-center gap-2 rounded-lg px-3 py-2.5 text-xs sm:text-sm ${status.type === "success"
                ...
                {moods.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center px-2.5 py-2 rounded-xl border text-[11px] sm:text-xs transition-colors ${selected
                        ? "border-emerald-500 bg-emerald-50 text-emerald-700"
                        : "border-gray-200 bg-gray-50 text-gray-600 hover:border-emerald-300 hover:bg-emerald-50/60"
                        }`}
                    >
                      <Icon
                        className={`w-5 h-5 mb-0.5 ${selected ? "text-emerald-600" : "text-gray-500"
                          }`}
                      />
